...
class TblServiceController extends Controller
{
	...
	public function actionForm()
	{
		if(isset($_GET['form']) && isset($_GET['id'])){
			...
			if(isset($_POST[$formModel['formModelName']])){
				...
				Yii::app()->user->setState('formModel', $model);
				if($model->validate())
					print_r($model);
	...
	public function actionCreatepdf(){
        ...
        $pdf->SetHeaderData(
			...
			"Office of the Zoning Administrator",'',array(255,255,255));
			...
			if(array_key_exists($data, $labels)){
				/*if($ctr==1)
					...
					if($labels[$data]=='(Specify)')
						$html = $html . '<tr><td>sdfsdf</td>';
					else if(array_key_exists($this->getArray($data), $model)){
						$html = $html .
