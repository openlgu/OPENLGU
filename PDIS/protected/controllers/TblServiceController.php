...
class TblServiceController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','manage','delete','deleteForm'),
				'users'=>array('@'),
			),
	...
	public function actionCheckStatus()
	{
		...
		if(isset($_GET['code']) && isset($_GET['id'])){
			if(isset($_POST['TblRequestFiles']) && isset($_POST['FileUpload'])){
				...
				if($file->validate()){
					...
					$directory = $this->generateValidFileName($dbfile->service);
					if(!file_exists(Yii::app()->basePath.'/uploads/'.$directory))
						mkdir(Yii::app()->basePath.'/uploads/'.$directory);
					if(!file_exists(Yii::app()->basePath.'/uploads/'.$directory.'/'.$dbfile->code))
						mkdir(Yii::app()->basePath.'/uploads/'.$directory.'/'.$dbfile->code);
					if(!TblRequestFiles::model()->exists(
						...
						if($dbfile->save()){
							$uploadedFile->saveAs(Yii::app()->basePath.'/uploads/'.$directory.'/'.$dbfile->code.'/'.$uploadedFile->name);
		...
		if($model)
			...
			Yii::app()->user->setFlash(
				TbHtml::ALERT_COLOR_ERROR,
				'<strong>Sorry, your request was NOT found.</strong> Please check the Service and Code you entered.');
			$this->redirect('checkStatus');
	...
	public function actionForms()
	{
		if(isset($_GET['id'])){
			...
			))
			{
				...
				$file = new FileUpload;
				$dbfile = new TblServiceForms;
				if(isset($_POST['TblServiceForms']) && isset($_POST['FileUpload'])){
					$dbfile->attributes = $_POST['TblServiceForms'];
					$dbfile->service = $_GET['id'];
					if($file->validate()){
						$uploadedFile = CUploadedFile::getInstance($file,'file');
						$dbfile->filename = substr($uploadedFile->name, 0, strrpos($uploadedFile->name,'.'));
						$dbfile->extension = '.'.$uploadedFile->extensionName;
						if(!file_exists(Yii::app()->basePath.'/sources/'.$uploadedFile->name)){
							$dbfile->save();
							$uploadedFile->saveAs(Yii::app()->basePath.'/sources/'.$uploadedFile->name);
						}else
							Yii::app()->user->setFlash(
								TbHtml::ALERT_COLOR_ERROR,
								'<strong>Error!</strong> The form already exists.');
							$this->redirect(array('forms', 'id'=>$_GET['id']));
					}
				}
				$this->render('forms',array('model'=>$model, 'file'=>$file, 'dbfile'=>$dbfile, 'service'=>$_GET['id']));
			}
			else
	...
	public function actionDeleteForm()
	{
		if(isset($_GET['id']) && isset($_GET['form'])){
			$model = TblServiceForms::model()->findByAttributes(array('service'=>$_GET['id'], 'name'=>$_GET['form']));
			if($model){
				if($model->delete()){
					unlink(Yii::app()->basePath.'/sources/'.$model->filename.$model->extension);
					$this->redirect(array('forms', 'id'=>$_GET['id']));
				}
			}
		}
	...
	public function actionCreatepdf(){
		...
		if(isset($_GET['form']) && $_GET['form'] == 'Business License Application Form' && $model){
